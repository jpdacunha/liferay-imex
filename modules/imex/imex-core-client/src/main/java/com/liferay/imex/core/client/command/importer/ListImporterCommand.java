...
public class ListImporterCommand extends ImportTrackerCommand implements ImexCommand {
	
	private static final Log _log = LogFactoryUtil.getLog(ListImporterCommand.class);
	
	private ImporterTracker trackerService;
	
	private final static String[] COLUMN_NAMES = {"Priority", "Bundle Name", "Description"}; 
	...
	public void li() {
		...
		if (importers != null && importers.size() > 0) {
			...
			for (Map.Entry<String ,ServiceReference<Importer>> entry  : importers.entrySet()) {
				...
				String priority = (String)serviceReference.getProperty("imex.component.priority");
